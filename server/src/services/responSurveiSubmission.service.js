...
const validateResponSurvei = async (survei) => {
  ...
  // Awalnya kita menolak jika tanggal_mulai belum tercapai atau sudah lewat tanggal_berakhir.
  // Untuk mempermudah responden langsung bisa menjawab setelah survei dipublish,
  // kita longgarkan aturan tanggal. Jika tetap ingin membatasi tanggal, gunakan validasi di sisi client.
  ...
  if (count >= survei.jumlah_responden) {
    throw {
      status: 400,
      message: "This survei has reached the maximum number of respondents",
    };
  }
};
...
const validateKriteria = (kriteria, profilResponden) => {
  ...
  const unmetCriteria = Object.entries(kriteria).reduce(
    (acc, [key, expected]) => {
      if (
        expected == null ||
...
export const getOrCreateDraft = async (surveiId, umumId) => {
  ...
  if (existingCompleted) {
    throw {
      status: 400,
      message:
        "You have submitted respon survei, cannot send more in this survei",
    };
  }
...
const validateResponKeys = async (pertanyaanSurveiList, respon) => {
  ...
  if (invalidKeys.length > 0) {
    throw {
      status: 400,
      message: `Invalid pertanyaan ID(s) in response: ${invalidKeys.join(
        ", "
      )}`,
    };
  }
...
const validateRequiredPertanyaan = async (pertanyaanSurveiList, respon) => {
  ...
  if (unanswered.length > 0) {
    throw {
      status: 400,
      message: `Required pertanyaan not answered: ${unanswered.join(", ")}`,
    };
  }
};
