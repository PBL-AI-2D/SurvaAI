...
export const getPublishedSurveis = async (req, res) => {
  try {
    // Longgarkan filter: tampilkan semua survei berstatus 'published'
    // tanpa memaksa tanggal_mulai/tanggal_berakhir harus mengandung hari ini.
...
export const getPublishedSurvei = async (req, res) => {
  try {
    ...
    resSuccess(
      res,
      `Survei details for ID ${req.params.id} retrieved successfully`,
      survei
    );
  } catch (error) {
    resFail(res, error.message, error.status);
